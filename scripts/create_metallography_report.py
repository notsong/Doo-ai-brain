...
def build_summary() -> pd.DataFrame:
    ...
    rows = [
        ...
        # The source sheet used the embedded/non-embedded labels in reverse;
        # the corrected mapping follows the user's clarification.
        ("脱碳层", "镶嵌-全脱", 61, 0.915, 0.925, 0.958),
        ("脱碳层", "镶嵌-总脱", 398, 0.941, 0.961, 0.970),
        ("脱碳层", "非镶嵌-全脱", 56, 0.920, 0.935, 0.967),
        ("脱碳层", "非镶嵌-总脱", 217, 0.912, 0.906, 0.953),
    ]
...
def build_report(output_dir: Path) -> Path:
    ...
    _add_section(
        ...
        chart_paths[3],
        "图 4  脱碳层四组工况指标对比",
        "镶嵌总脱的 IoU/P/R 为 0.941/0.961/0.970，高于非镶嵌总脱的 0.912/0.906/0.953，符合镶嵌样品总脱边缘更清晰、过渡范围更易定位的特点；非镶嵌总脱边缘不明显，更容易受组织渐变、背景纹理和光照波动影响，造成定位偏差和误检。全脱方面，本批镶嵌全脱的 IoU/P/R 为 0.915/0.925/0.958，略低于非镶嵌全脱的 0.920/0.935/0.967，差异主要与样本数量和局部组织、边缘质量差异有关。",
    )

    doc.add_heading("总体验收结论", level=1)
    _add_paragraph(doc, "根据本次填写的真实性能指标数据，夹杂物、晶粒度和脱碳层三个检测项目的 7 个验证单元，其 IoU、P、R 三项指标均达到设定阈值。")
    _add_paragraph(doc, "从整体指标关系看，R 普遍高于 P，说明系统对目标的覆盖能力较好，但仍存在一定程度的误检、边界外扩或区域归属偏差；后续优化重点应放在条状夹杂物边界精修、非镶嵌总脱的弱边缘判定及非镶嵌样品边缘伪影抑制。")
