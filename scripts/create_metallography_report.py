...
OUTPUT_NAME = "AI金相检测项目性能验证报告_模拟数据.docx"
...
def build_summary() -> pd.DataFrame:
    """Return the seven fixed, clearly simulated validation results."""
    ...
    frame = pd.DataFrame(rows, columns=["project", "label", *METRICS])
...
def _plot_grouped_bars(frame: pd.DataFrame, title: str, path: Path) -> None:
    ...
    ax.set_title(f"{title}（模拟验证数据）", fontsize=14, fontweight="bold", **_font_kwargs())
...
def _add_metric_table(doc: Document, frame: pd.DataFrame) -> None:
    ...
    for project, label, iou, precision, recall, passed in frame.itertuples(index=False, name=None):
        cells = table.add_row().cells
        values = [label, f"{iou:.3f}", f"{precision:.3f}", f"{recall:.3f}", "是" if passed else "否"]
        for cell, text in zip(cells, values):
            paragraph = cell.paragraphs[0]
            paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
            run = paragraph.add_run(text)
            _set_run_font(run, size=10)
    _set_table_widths(table, (2.35, 0.9, 0.9, 0.9, 1.35))
...
def _add_section(doc: Document, heading: str, frame: pd.DataFrame, chart_path: Path, chart_caption: str) -> None:
    doc.add_heading(heading, level=1)
    _add_paragraph(doc, "本节采用模拟验证数据展示该检测项目的 IoU、Precision（P）和 Recall（R）指标。")
    _add_metric_table(doc, frame)
    _add_chart(doc, chart_path, chart_caption)
    _add_paragraph(doc, "模拟数据结果显示，本节所列验证单元的三项指标均达到设定阈值。")
...
def build_report(output_dir: Path) -> Path:
    ...
    notice_run = notice.add_run("说明：本报告全部数据为模拟验证数据，仅用于展示报告格式与指标呈现方式。")
    _set_run_font(notice_run, size=10, bold=True, color=(155, 28, 28))
    doc.add_page_break()
    ...
    _add_paragraph(doc, "本报告仅采用标准中常用的三项指标进行展示：IoU 表示预测区域与参考区域的交并比；P（Precision）表示预测为目标区域的结果中实际正确的比例；R（Recall）表示参考目标区域被正确识别的比例。")
    _add_paragraph(doc, "本报告采用的展示阈值为：IoU ≥ 0.85，P ≥ 0.90，R ≥ 0.95。表格中的“是”表示该验证单元的三项指标均达到对应阈值。")

    doc.add_heading("执行摘要", level=1)
    _add_paragraph(doc, "下表为 7 个验证单元的 IoU、P、R 模拟汇总结果。")
    _add_metric_table(doc, summary)
    _add_chart(doc, chart_paths[0], "图 1  三个检测项目核心指标总览（模拟验证数据）")

    _add_section(doc, "夹杂物性能验证", summary.iloc[:2], chart_paths[1], "图 2  夹杂物条状/点状指标对比（模拟验证数据）")
    _add_section(doc, "晶粒度性能验证", summary.iloc[2:3], chart_paths[2], "图 3  晶界指标结果（模拟验证数据）")
    _add_section(doc, "脱碳层性能验证", summary.iloc[3:], chart_paths[3], "图 4  脱碳层四组工况指标对比（模拟验证数据）")

    doc.add_heading("总体验收结论", level=1)
    _add_paragraph(doc, "在本报告设定的模拟验证数据和评价阈值下，夹杂物、晶粒度和脱碳层三个检测项目的 7 个验证单元，其 IoU、P、R 三项指标均达到设定阈值。")
    _add_paragraph(doc, "以上结论仅用于展示客户验收报告的指标组织方式，不代表真实项目测试结果；实际验收时应替换为经双方确认的测试集和实测数据。")
